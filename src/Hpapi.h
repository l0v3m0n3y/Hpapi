...
class Hpapi {
private:
    std::string api_base = "https://hp-api.onrender.com/api";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("hp-api.onrender.com"));
...
public:
    ...
    pplx::task<json::value> characters_list() {
        return make_api_call("/characters","GET");
    }

    pplx::task<json::value> characters_in_gryffindor(const std::string& title) {
        return make_api_call("/characters/house/gryffindor","GET");
    }

    pplx::task<json::value> characters_staff() {
        return make_api_call("/characters/staff","GET");
    }

    pplx::task<json::value> character_by_id(const std::string& character_id) {
        return make_api_call("/character/" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(character_id))),"GET");
    }
};
